...
const ScrollStack = ({
  ...
  useLayoutEffect(() => {
    ...
    // ── Scroll boundary hand-off ──────────────────────────────────────────────
    // Find the real page-level scroll container (the snap-y <main>), not window
    ...
    const pageScroller = findScrollParent(scroller);
    const wheelTarget  = scroller.parentElement ?? document.body;

    const handleWheel = e => {
      const lenis = lenisRef.current;
      if (!lenis) return;

      const scrollTop   = lenis.scroll ?? scroller.scrollTop;
      const scrollLimit = lenis.limit  ?? (scroller.scrollHeight - scroller.clientHeight);
      const delta       = e.deltaY;

      const atTop    = scrollTop <= 1 && delta < 0;
      const atBottom = scrollTop >= scrollLimit - 1 && delta > 0;

      if (atTop || atBottom) {
        e.stopImmediatePropagation();
        e.preventDefault();
        // Delegate to the real scroll container (the snap <main>), not window
        if (pageScroller === window) {
          window.scrollBy({ top: delta, behavior: 'smooth' });
        } else {
          pageScroller.scrollBy({ top: delta, behavior: 'smooth' });
        }
      }
    };

    // Capture phase on the *parent* — fires before Lenis's capture listener on the child
